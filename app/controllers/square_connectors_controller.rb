class SquareConnectorsController < ApplicationController
  ...
  before_action :find_square_connector, only: %i[ callback]
  def index
  ...
  def create
    ...
    respond_to do |format|
      if @square_connector.save
        format.html { redirect_to "https://connect.squareupsandbox.com/oauth2/authorize?client_id=#{@square_connector.client_id}&scope=ORDERS_WRITE%20ORDERS_READ%20BANK_ACCOUNTS_READ%20MERCHANT_PROFILE_READ%20PAYMENTS_READ%20SETTLEMENTS_READ"}
  ...
  def callback
    ...
    if authorization_code
      SquareService.new(@square_connector, authorization_code).call
      respond_to do |format|
  ...
  private
    ...
    def square_connector_params
      params.require(:square_connector).permit(:client_id, :client_secret)
    end
end
